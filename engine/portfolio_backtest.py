...
class CostModel:
    """Trading-cost assumptions for Taiwan cash equities.

    Commission is configurable because brokers can offer discounts.
    Defaults use the commonly referenced 0.1425% commission rate and the
    current 0.3% stock transaction tax for non-day-trading stock sales.
    Slippage is an explicit research assumption, not a broker quote.
    """
...
def _sell_cash_proceeds(price: float, quantity: float, costs: CostModel) -> float:
    ...
    notional = executed * quantity
    commission = notional * costs.sell_commission
    tax = notional * costs.sell_tax
    return notional - commission - tax


def _liquidation_value(price: float, quantity: float, costs: CostModel) -> float:
    return _sell_cash_proceeds(price, quantity, costs)
...
def simulate_portfolio(
    ...
    """Run a capital-constrained portfolio simulation over single-stock trades.

    The single-stock strategy remains unchanged. This layer adds:
    - max concurrent positions
    - per-position capital allocation
    - cash accounting
    - transaction costs and slippage
    - daily marked-to-market equity
    - score-based selection when too many signals arrive together
    """
    ...
    df = df.dropna(subset=["stock_id", "entry_date", "exit_date", "entry", "exit", "score"]).copy()
    df["stock_id"] = df["stock_id"].astype(str)
    df = df.sort_values(["entry_date", "score", "stock_id"], ascending=[True, False, True]).reset_index(drop=True)
    ...
    prices = prices.dropna(subset=["stock_id", "date", "close"])
    close_lookup = prices.set_index(["date", "stock_id"])["close"]

    start_date = min(df["entry_date"].min(), df["exit_date"].min())
    end_date = max(df["entry_date"].max(), df["exit_date"].max())
    dates = pd.date_range(start_date, end_date, freq="B")

    entries_by_date = {date: group.to_dict("records") for date, group in df.groupby("entry_date", sort=True)}
    exits_by_date = {date: group.to_dict("records") for date, group in df.groupby("exit_date", sort=True)}
    ...
    for date in dates:
        # Existing positions scheduled to exit today are closed before normal new entries.
        exit_ids = [tid for tid, pos in positions.items() if pos["exit_date"] == date]
        for tid in exit_ids:
            pos = positions.pop(tid)
            proceeds = _sell_cash_proceeds(pos["exit_price"], pos["quantity"], costs)
            cash += proceeds
            ...
            fills.append(
                {
                    ...
                    "gross_return_pct": (pos["exit_price"] / pos["entry_price"] - 1.0) * 100.0,
                    "net_pnl": pnl,
        ...
        for record in selected:
            stock_id = str(record["stock_id"])
            if any(pos["stock_id"] == stock_id for pos in positions.values()):
                rejected.append(
                    {
                        "trade_id": int(record["trade_id"]),
                        "entry_date": date,
            ...
            unit_cash = float(record["entry"]) * (1.0 + costs.buy_slippage) * (1.0 + costs.buy_commission)
        ...
        for pos in positions.values():
            close = close_lookup.get((date, pos["stock_id"]), np.nan)
            if pd.isna(close):
                close = pos["entry_price"]
            close = float(close)
            exposure += close * pos["quantity"]
            equity += _liquidation_value(close, pos["quantity"], costs)
    ...
    metrics = _portfolio_metrics(
        equity_curve, fills_df, rejected_df,
        initial_capital, allocation_pct, max_positions, costs,
    )
...
def _portfolio_metrics(
    ...
    elapsed_days = max((equity_curve["date"].iloc[-1] - equity_curve["date"].iloc[0]).days, 1)
        ...
        result.update(
            {
                ...
                "avg_holding_days": round(
                    float((pd.to_datetime(fills["exit_date"]) - pd.to_datetime(fills["entry_date"])).dt.days.add(1).mean()),
                    2,
                ),
            }
        )
