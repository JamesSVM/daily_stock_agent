...
"""Build V1.6 Top-1/3/5/10 signal-event performance reports."""
...
def _build_daily_rankings(stock_data: dict[str, pd.DataFrame]) -> list[pd.DataFrame]:
            ...
            by_date.setdefault(signal_date, []).append(
                {
                    ...
                    "stock_id": stock_id,
                    "market_regime": str(market.loc[signal_date, "regime"])
    ...
    for signal_date in sorted(by_date):
        daily = pd.DataFrame(by_date[signal_date])
        rankings.append(rank_daily_signals(daily, top_n=10))
    return rankings


def _load_taiwan_benchmark(period: str = "3mo") -> pd.DataFrame:
    """Use TAIEX when available; the tracker can still operate without it."""
    try:
    ...
    except Exception as exc:  # noqa: BLE001 - benchmark is supplemental
...
def run(db_path: str, benchmark_period: str = "3mo") -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    events = build_signal_events(rankings, stock_data, benchmark=benchmark)
