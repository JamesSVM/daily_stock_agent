...
def build_signal_sheet(stock_data: dict[str, pd.DataFrame]) -> tuple[pd.DataFrame, str]:
    ...
    for stock_id, df in stock_data.items():
        ...
        candidate = (
            latest_regime in {"bull", "neutral"}
            and rs20 > RS_THRESHOLD
            and PULLBACK_MIN <= drawdown <= PULLBACK_MAX
            and score >= MIN_SCORE
            and bool(signal["trend_pass"])
            and bool(signal["momentum_pass"])
        )
